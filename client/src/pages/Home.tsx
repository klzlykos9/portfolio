          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_480px] gap-12 lg:gap-16 items-center">
            ...
            >
              <div className="relative">
                {/* Ambient glow */}
                <div className="absolute -inset-8 bg-cyan-500/5 rounded-[2.5rem] blur-3xl pointer-events-none" />
                <div className="absolute -inset-4 bg-violet-600/4 rounded-[2.5rem] blur-2xl pointer-events-none" />

                {/* ── Card ── */}
                <div className="relative rounded-3xl overflow-hidden shadow-2xl"
                     style={{
                       background: 'linear-gradient(145deg, rgba(15,20,40,0.95) 0%, rgba(8,12,26,0.98) 100%)',
                  ...
                  {/* ── Header ── */}
                  <div className="px-7 pt-7 pb-6 flex items-start justify-between">
                    {/* Avatar + identity */}
                    <div className="flex items-center gap-4">
                      {/* Monogram */}
                      <div className="relative shrink-0">
                        <div className="w-14 h-14 rounded-2xl flex items-center justify-center font-black text-lg text-white select-none"
                             style={{ background: 'linear-gradient(135deg, #0891b2 0%, #6366f1 100%)' }}>
                          AN
                        </div>
                        {/* Online dot */}
                        <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-emerald-400 border-2 border-[#08091a] rounded-full">
                          <span className="absolute inset-0 rounded-full bg-emerald-400 animate-ping opacity-75" />
                        </span>
                      </div>

                      <div>
                        <h3 className="text-white font-black text-base leading-tight tracking-tight">Arpan P. Nayak</h3>
                        <p className="text-slate-400 text-[11px] mt-0.5 font-medium">AI Engineer · Business Strategist</p>
                        <div className="flex items-center gap-1.5 mt-1.5">
                          <MapPin size={9} className="text-slate-600" />
                          <span className="text-[10px] text-slate-600 font-medium">India</span>
                          <span className="text-slate-700 mx-0.5">·</span>
                          <Globe size={9} className="text-slate-600" />
                          <span className="text-[10px] text-slate-600 font-medium">Open to Remote</span>
                        </div>
                      </div>
                    </div>

                    {/* Available badge */}
                    <div className="px-2.5 py-1 rounded-full border border-emerald-500/30 bg-emerald-500/8 flex items-center gap-1.5 shrink-0">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                      <span className="text-[9px] text-emerald-400 font-black uppercase tracking-widest">Available</span>
                    </div>
                  </div>
                  ...
                  {/* ── Stats ── */}
                  <div className="px-7 py-5 grid grid-cols-3 gap-0">
                    {[
                      { val: '20+', label: 'Projects', color: 'text-cyan-400' },
                      { val: '10+', label: 'AI Systems', color: 'text-violet-400' },
                      { val: '5+',  label: 'Certs',      color: 'text-amber-400' },
                    ].map((s, i) => (
                      <div key={i} className={`text-center ${i < 2 ? 'border-r border-white/6' : ''}`}>
                        <div className={`text-xl font-black ${s.color}`}>{s.val}</div>
                        <div className="text-[10px] text-slate-600 mt-0.5 font-medium uppercase tracking-wider">{s.label}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="px-7 py-5">
                    <p className="text-[9px] text-slate-600 font-black uppercase tracking-[0.22em] mb-3">Current Focus</p>
                    <div className="space-y-2">
                      {FOCUS_ITEMS.map((item, i) => (
                        <motion.div
                          ...
                          className={`flex items-center gap-2.5 px-3 py-2 rounded-xl border ${item.color} text-[11px] font-semibold`}
                        >
                          <CheckCircle2 size={11} className="shrink-0 opacity-70" />
                          {item.label}
                        </motion.div>
                      ))}
                    </div>
                  ...
                  {/* ── Stack ── */}
                  <div className="px-7 py-5">
                    <p className="text-[9px] text-slate-600 font-black uppercase tracking-[0.22em] mb-3">Core Stack</p>
                    <div className="flex flex-wrap gap-1.5">
                      {STACK_TAGS.map(tag => (
                        <span key={tag}
                          className="px-2.5 py-1 rounded-lg bg-white/4 border border-white/8 text-slate-400 text-[10px] font-bold hover:text-white hover:border-white/15 transition-colors">
                          {tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="px-7 py-4 flex items-center justify-between">
                    <div className="flex gap-2.5">
                      {[
                        ...
                        <a key={label} href={href} target="_blank" rel="noreferrer" aria-label={label}
                           className="p-2 rounded-lg bg-white/4 border border-white/8 text-slate-500 hover:text-white hover:bg-white/8 hover:border-white/15 transition-all duration-200">
                          <Icon size={13} />
                        </a>
                      ))}
                    </div>
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-cyan-500/15 border border-cyan-500/30 text-cyan-300 text-[11px] font-black hover:bg-cyan-500/25 hover:border-cyan-400/50 transition-all flex items-center gap-1.5"
                    >
                      Hire Me <ArrowRight size={10} />
                    </button>
                  </div>
                ...
                {/* Floating tech chips — kept from original */}
                <motion.div
                  animate={{ y: [0, -10, 0] }}
                  transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut' }}
                  className="absolute -top-4 -right-4 px-3.5 py-2.5 rounded-xl backdrop-blur-md shadow-lg border"
                  style={{ background: 'rgba(109,40,217,0.25)', borderColor: 'rgba(139,92,246,0.35)' }}
                >
                  <p className="text-violet-200 text-xs font-black">LangGraph</p>
                  <p className="text-violet-400/60 text-[10px]">Multi-Agent</p>
                </motion.div>

                <motion.div
                  animate={{ y: [0, 10, 0] }}
                  transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut', delay: 1.2 }}
                  className="absolute -bottom-4 -left-4 px-3.5 py-2.5 rounded-xl backdrop-blur-md shadow-lg border"
                  style={{ background: 'rgba(8,145,178,0.20)', borderColor: 'rgba(6,182,212,0.35)' }}
                >
                  <p className="text-cyan-200 text-xs font-black">RAG Pipeline</p>
                  <p className="text-cyan-400/60 text-[10px]">Production</p>
                </motion.div>

                <motion.div
                  animate={{ y: [0, -7, 0] }}
                  transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut', delay: 2 }}
                  className="absolute top-1/2 -right-6 px-3 py-2 rounded-xl backdrop-blur-md shadow-lg border"
                  style={{ background: 'rgba(180,83,9,0.20)', borderColor: 'rgba(245,158,11,0.30)' }}
                >
                  <p className="text-amber-200 text-xs font-black">Six Sigma</p>
                  <p className="text-amber-400/60 text-[10px]">Black Belt</p>
                </motion.div>
              </div>
            </motion.div>
          </div>
