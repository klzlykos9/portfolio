  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ message: userText, user_name: userName, current_page: location.pathname }),
      });
      const data = await res.json();
      setIsTyping(false);
      addMessage({ id: Date.now().toString(), text: data.response, sender: 'ai' });
    } catch (error) {
      setIsTyping(false);
      addMessage({ id: 'err', text: "Connection error. My neural link is weak!", sender: 'ai' });
    }
  };
